...
- [ ] **PROC-03**: Child-process containment (killing a provider CLI kills its descendants, not just the parent) works on macOS for every process spawn path, including `codexrun.py::CodexProcess.close()` where `start_new_session` is currently missing — matching the pattern already correct in `gitrunner.py` (codexrun.py/antigravityrun.py done in 02-01; supervisor.py/mailhub_runtime.py remain — 02-02)
...
| PROC-03 | Phase 2 | In Progress (02-01 of 2 plans done) |
